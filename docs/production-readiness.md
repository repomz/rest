...
Это честная оценка того, что в `rest_generator` уже выглядит крепко, а что стоит реализовать до уровня зрелого production-ready open-source проекта.
...
| Safety | `rest init` не перезаписывает существующие config-файлы |
...
- добавить `rest generate --dry-run`;
