...
- включение SQLC и настройку `sqlc_path` для режимов `rest init --sqlc` и `rest init --example`;
...
- при `auto_sqlc: enable` выполнить `sqlc generate -f <sqlc_path>`;
...
- завершить генерацию командой `go mod tidy`.
...
5. Read SQLC `querier.go` metadata.
6. Read SQLC parameter structs.
7. Detect optional query params из `sqlc.narg(...)`.
8. Build endpoint metadata и attach endpoints к tables.
9. Собрать полный список generated files.
10. При включенном `safe_reload` сравнить файлы с предыдущим снимком и запросить решение по пользовательским изменениям.
11. Clean generated app layers и удалить выключенные optional files.
12. Сгенерировать optional Goose init migration.
13. Render shared и per-table files.
14. Write OpenAPI output, если включен.
15. Сохранить новый safe reload snapshot и восстановить выбранные пользовательские версии.
...
| Файл | Что генерирует |
| --- | --- |
| `templates_app.go` | `cmd/main.go`, Makefile, init DB, CI/CD workflows и entrypoint behavior |
| `templates_common.go` | app config и common HTTP response/error helpers |
| `templates_domain.go` | domain models, validation, DB-to-domain conversion |
| `templates_http.go` | handlers, DTOs и HTTP server |
| `templates_features.go` | Docker, metrics, env и gitignore files |
| `templates_testing.go` | generated handler tests |
| `feature_templates.go` | middleware, logging и Swagger/OpenAPI serving |
...
При `safe_reload: disable` ручные изменения этих файлов будут потеряны. При `safe_reload: enable` генератор показывает diff и предлагает сохранить или перезаписать пользовательскую версию каждого измененного файла.
